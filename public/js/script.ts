$(function () {
    ...
    function confirmPassword () {
        const passwordVal = $password.val() as string;
        if (passwordVal.length === 0) return;

        const confirmVal = $confirmation.val() as string;
    ...
    function updateProgress (evt : JQuery.ChangeEvent<HTMLElement>) {
    ...
    function toggleForms (selectors : JQuery<HTMLElement>[]) {
    ...
    function toggleFormsBasedOnMovementType (eventTrigger : JQuery<HTMLElement>, targetForm : JQuery<HTMLElement>) {
    ...
    function enableSubmit (evt : JQuery.ChangeEvent<HTMLElement>) {
    ...
    function handleWorkoutFormChange (this : HTMLElement) {
        const $exerciseSection = $(this).closest('.workoutInputChild');
        const selectedOption = $(this).find(':selected');
        const type = selectedOption.data('type') as 'weighted' | 'cardio';
    ...
    function validateWorkoutForm (type : 'weighted' | 'cardio', weighted : JQuery<HTMLElement>, cardio : JQuery<HTMLElement>) {
        if (type === 'weighted') {
            weighted.show();
            cardio.hide();
    ...
    function enableWorkoutSubmit () {
        ...
        const allFilled = $inputs.toArray().every(function (input) {
            return ($(input).val() as string).trim() !== '';
        });
        $('.workoutSubmit').prop('disabled', !allFilled);
    }

    function toggleCollapse (evt : JQuery.ClickEvent<HTMLElement>) {
        if (evt.target.id === 'pendingCollapse') $pending.slideToggle();
        if (evt.target.id === 'friendsCollapse') $friend.slideToggle();
    }

    async function togglePublic (evt : JQuery.ClickEvent<HTMLElement>) {
